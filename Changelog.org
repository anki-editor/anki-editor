...
** Unreleased
   ...
   13. Replaced macros ~anki-editor-api-with-multi~ and
       ~anki-editor-api-enqueue~. Now ~anki-editor-api-enqueue-request~
       queues an action with success/error callbacks, and
       ~anki-editor-api-dispatch-queue~ sends the queued requests as a
       single ~multi~ call and dispatches the callbacks against the
       responses
   14. Media links are now handled directly by the export backend.
